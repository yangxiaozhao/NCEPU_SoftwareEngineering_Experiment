...
public class ReturnBook extends JInternalFrame {
	...
	public ReturnBook() {
		setTitle("还书业务");
		setFrameIcon(new ImageIcon(ReturnBook.class.getResource("/images/办理业务.png")));
		setClosable(true);
		...
		JButton btnNewButton = new JButton("还   书");
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					ee=Service.returnbook(rid, sno, bno, bor, time);
					if(ee.equals(""))
		...
		bnametext.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Vector v=new Vector<>();
				try {
					v = Service.getrentbno(bnametext.getText());
